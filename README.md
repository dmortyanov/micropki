...
### `ca issue-ocsp-cert` — Выпуск сертификата для OCSP
...
| `--subject`          | Distinguished Name                              | —            |
| `--key-type`         | `rsa` или `ecc`                                 | `rsa`        |
| `--key-size`         | Размер ключа: 2048/4096 (RSA) или 256/384 (ECC) | 2048 / 256   |
| `--out-dir`          | Выходная директория                             | `./pki/certs`|
| `--validity-days`    | Срок действия (дни)                             | 365          |
| `--log-file`         | Путь к лог-файлу                                | stderr       |

### `ocsp serve` — Запуск OCSP Responder

| Параметр             | Описание                                  | По умолчанию |
|----------------------|-------------------------------------------|--------------|
| `--db`               | Путь к SQLite базе данных                 | `micropki.db`|
| `--ca-cert`          | Сертификат CA-эмитента (PEM)              | —            |
| `--responder-cert`   | Сертификат OCSP responder (PEM)           | —            |
| `--responder-key`    | Приватный ключ responder (незашифрованный)| —            |
| `--host`             | Хост, на котором будет запущен сервер     | `127.0.0.1`  |
| `--port`             | Порт сервера                              | `8080`       |
| `--cache-ttl`        | Значение NextUpdate TTL в секундах        | `60`         |
| `--ocsp-log`         | Файл для аудита запросов (JSONL)          | `ocsp.jsonl` |
| `--log-file`         | Путь к лог-файлу сервера                  | stderr       |
